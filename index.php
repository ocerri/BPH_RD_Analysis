...
div.pic {
    ...
    text-align: center;
    margin: 7px 0.5%;
    -moz-box-shadow: 5px 3px 3px rgb(80,80,80);    /* Firefox 3.5 */
    -webkit-box-shadow: 5px 3px 3px rgb(80,80,80); /* Chrome, Safari */
    box-shadow: 5px 3px 3px rgb(80,80,80);         /* New browsers */
}
...
$plotswidth = htmlspecialchars($_GET['plotswidth']);
if (!isset($plotswidth) || empty($plotswidth)) {$plotswidth="30";}
$noplots = htmlspecialchars($_GET['noplots']);
?>
<h1><?php echo getcwd(); ?></h1>
<div style="display: block; clear:both;">
<p><form>
  Filter: <input type="text" name="match" size="30" value="<?php if (isset($match)) print $match;  ?>" /><br>
  Plots width [%]: <input type="text" name="plotswidth" size="10" value="<?php print $plotswidth;  ?>" /><br>
  <input type="Submit" value="Set" />
</form></p>
<h2><a name="directories">Directories</a></h2>
<ul>
<li><a href="../index.php">[DIR] ..</a></li>
<?php
...
<div style="display: block; clear:both;">
<h2><a name="text_files">Text files</a></h2>
<ul>
<?php
foreach (glob("*") as $filename) {
    if ($noplots || !in_array($filename, $displayed)) {
        if (isset($match) && !fnmatch('*'.$match.'*.txt', $filename)) continue;
        if (!is_dir($filename)) {
...
if ($noplots) {
    ...
    foreach ($filenames as $filename) {
        ...
        print "<div class='pic' style=\"width: $plotswidth%;\">\n";
...
foreach (glob("*") as $filename) {
    if ($noplots || !in_array($filename, $displayed)) {
        if (fnmatch('*.txt', $filename)) continue;
        if (isset($match) && !fnmatch('*'.$match.'*', $filename)) continue;
        if (!is_dir($filename)) {
            print "<li><a href=\"$filename\">$filename</a></li>";
        }
