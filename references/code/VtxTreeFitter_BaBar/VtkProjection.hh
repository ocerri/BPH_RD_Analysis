...
namespace vtxtreefit
{
  ...
  public:
    ...
    HepDouble& V(int row, int col) { return _matrixV(_offset+row,_offset+col) ; }
    HepDouble& Vfast(int row, int col) { 
#ifdef VTK_BOUNDSCHECKING
    ...
    void reset() {
      ...
      memset(&(_matrixH(1,1)),0,dimP*dimC*sizeof(HepDouble));
      memset(&(_r(1))  ,0,dimC*sizeof(HepDouble));
      memset(&(_matrixV(1,1)),0,dimC*(dimC+1)/2*sizeof(HepDouble));
      _offset = 0 ;
    }
