...
namespace vtxtreefit
{
  ...
  void VtkHelixUtils::vertexFromHelix(const HepVector& helixpar, const BField& fieldmap,
				      HepVector& vertexpar, int& charge)
  {
    ...
    const double Bz = fieldmap.bFieldNominal() ;
    const double a  = -BField::cmTeslaToGeVc*Bz ;
    charge = omega>0 ? -1 : 1 ;
				      ...
				      HepVector& helixpar, HepMatrix& jacobian) 
  {
    ...
    else if(deltaphi < -Constants::pi ) deltaphi += Constants::twoPi ;
  ...
  {
    double rc = phi ;
    if     (phi < -Constants::pi)  rc += Constants::twoPi ;
    else if(phi >  Constants::pi)  rc -= Constants::twoPi ;
    return rc ;
  }
...
//  	 << x0_2 << "," << y0_2 << "," << r_2 << endl ;
	...
	double l1 = (dphi1 + n1*Constants::twoPi)/omega_1 ;
	double tmpz1 = (z0_1 + l1*tandip_1) ;
	if( n1==0 || fabs(tmpz1)<100 ) {
	  for(int n2 = 1-ncirc ; n2 <=1+ncirc; ++n2) {
	    double l2 = (dphi2 + n2*Constants::twoPi)/omega_2 ;
				  ...
				  double& flt)
  {
    ...
    for(int n = 1-ncirc; n <=1+ncirc ; ++n) {
      double l = (dphi + n*Constants::twoPi)/omega ;
